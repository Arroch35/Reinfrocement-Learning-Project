...
# In this code when we start climb up the ladder,
# we do up action 35 times
...
#
# Output:
# - Modifies the original reward by adding intermediate rewards based on the agent's level and position.
    def __init__(self, env):
        """
        Initializes the wrapper with custom variables for tracking the agent's progress.
    ...
    def step(self, action):
        """
        Takes a step in the environment, modifies the reward based on the agent's state,
        and returns the observation, reward, done status, truncated status, and info.

        Parameters:
        - action: The action taken by the agent.

        Returns:
        - obs: The environment's observation after the action.
        - reward: The modified reward after applying additional rewards.
        ...
        # Check if agent level is valid and within the range
        if agent_level is not None and 1 <= agent_level <= len(self.ladder_postion):
            agent_level_reward = (7 - agent_level)* (-0.01) # add level based reward
            additional_reward += agent_level_reward

            # Check if agent has reached a new level
            if agent_level > self.last_level:
                additional_reward += 500.0 # add reward for reaching a new level
            ...
            if agent_position is not None and len(agent_position) > 0:
                ...
                # if agent is climbing up the ladder, add additional reward
                # add we execute up action 35 times, in order to climb up the ladder
...
eval_callback = EvalCallback(
    ...
    eval_freq=250000,                            # evaluate the model every 250,000 steps
    deterministic=True,
    render=False
)
