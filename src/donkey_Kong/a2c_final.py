...
#
# Output:
# - Transposed image observations in the format (n_envs, Channels, Height, Width).
...
def get_agent_level_position(image):
    """
        Transposes a single observation from (n_envs, H, W, C) to (n_envs, C, H, W).

        Parameters:
        - obs: A numpy array representing the observations.
...
eval_callback = EvalCallback(
    ...
    eval_freq=10000,                            # evaluate the model every 10,000 steps
    deterministic=True,
    render=False
)
